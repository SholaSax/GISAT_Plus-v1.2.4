...
(SELECT  @row_number :=CASE WHEN @patient_id = patient_id THEN @row_number + 1 ELSE 1 END AS num, @patient_id := patient_id AS patient_id,
DATE_FORMAT(encounter_datetime,'%Y-%m-%d') AS encounter_datetime,encounter_id ,voided FROM encounter WHERE encounter_type = 10 AND encounter_datetime BETWEEN @startdate AND @Reporting_Date AND voided = 0
...
DATE_FORMAT(encounter_datetime,'%Y-%m-%d') AS encounter_datetime,encounter_id ,voided FROM encounter WHERE (encounter_type = 36 OR encounter_type = 35) AND encounter_datetime BETWEEN @startdate AND @Reporting_Date AND voided = 0
...
WHERE ChildRegistraion.encounter_datetime BETWEEN @startdate AND @Reporting_Date
...
cur.execute("""
  ...
  BEGIN
SELECT 'CIHP' IP,
(SELECT State FROM CIHP_ListOfFacility WHERE Datim_Code = (SELECT `property_value` FROM `global_property`
WHERE `property`= 'facility_datim_code')) AS State,
...
(SELECT property_value FROM global_property WHERE property='Facility_Name') AS FacilityName,
(SELECT property_value FROM global_property WHERE property='facility_datim_code') AS DatimCode,
pid.`identifier` AS PID ,
padd.`latitude`,
padd.`longitude`,
IFNULL(bo.new_template, bo.template) AS 'final_template',
bo.date_created,
bo.imageQuality,
bo.fingerPosition,
MD5(bo.date_created) AS 'Versioning'
FROM patient_identifier pid 
LEFT JOIN (SELECT DISTINCT person_id, latitude, longitude FROM person_address WHERE voided=0) AS padd ON pid.patient_id = padd.person_id
LEFT JOIN `biometricinfo` bo ON pid.`patient_id` = bo.`patient_Id`
WHERE padd.`latitude` IS NOT NULL AND padd.`longitude` IS NOT NULL
AND pid.identifier_type=8
AND pid.voided=0;
  END
""")
...
SELECT 
'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName,
gender, 
        identifier, 
        pni.patient_id AS 'Db_Patient Id',
        e.encounter_datetime AS "ART Commencement Date",
        bi.date_created AS 'Capture Date',
CASE 
WHEN bi.date_created IS NULL THEN 'Not Captured'
WHEN bi.date_created IS NOT NULL THEN "Captured" 
END AS "Feedback"
FROM 
(SELECT p.person_id, given_name, family_name, gender, identifier, patient_id 
FROM person p
...
AND p.voided = 0 AND pn.voided = 0)pni
JOIN
(SELECT * FROM encounter 
WHERE encounter_datetime <= @Reporting_Date
AND encounter_type = 25 
AND form_id = 56
AND voided = 0) e
ON e.patient_id = pni.patient_id
LEFT JOIN
(SELECT * FROM biometricinfo
GROUP BY patient_Id
)bi
ON pni.patient_id = bi.patient_id 
